...
> **Status (2026-07-10, second pass): v2 complete.** Phases 0–4 are done: 49 components + 5 effects in `Sources/Swiftcn/Components|Effects/`, 5 blocks (login-01, settings-01, sidebar-07, dashboard-01, chat-01), the Showcase app catalogs all 60 registry items, `registry.json` is generated by `scripts/generate_registry.py`, and Phase 5's **CLI shipped its M1** (`cli/` — `swiftcn list/view/add/init` with transitive dependency resolution, local + GitHub-raw registries). An Apple-API audit confirmed zero deprecated APIs and zero iOS 17/macOS 14 floor violations. Remaining (deliberate cuts): Data Table pattern doc, Menubar doc page, CLI M2+ (swiftcn.json config, prefix rewriting, third-party namespaces), snapshot tests, Mintlify docs refresh, dashboard-01 data table section.
