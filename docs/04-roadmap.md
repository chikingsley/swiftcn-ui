...
> **Status (2026-07-10): v2 shipped.** Phases 0–2 are done in full, plus most of Phase 3's quick wins: 40 components + 2 effects live in `Sources/Swiftcn/Components|Effects/`, 4 blocks in `Blocks/` (login-01, settings-01, sidebar-07, dashboard-01), the Showcase app (`Showcase.swiftpm`) catalogs all 45 registry items, and `registry.json` is generated by `scripts/generate_registry.py`. Still open from the list below: Command (⌘K), Combobox, Calendar + Date Picker, Carousel, Table/Data Table, the chat suite, Tooltip, Hover Card, Pagination, Resizable, Menubar, more effects — and Phase 5's CLI (M1+; M0's registry.json exists).
