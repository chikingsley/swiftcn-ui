...
#Preview("ScrubBar") {
    @Previewable @State var value: TimeInterval = 30
...
#Preview("ScrubBar · track only") {
    @Previewable @State var value: TimeInterval = 96
    SCPreview {
        SCScrubBarContainer(
            duration: 240, value: value, onScrub: { value = $0 },
            content: {
                SCScrubBarTrack {
                    SCScrubBarProgress()
                }
...
#Preview("ScrubBar · scrubbing state") {
    @Previewable @State var value: TimeInterval = 45
    ...
    SCPreview {
        VStack(spacing: 12) {
            ...
            Text(isScrubbing ? "Scrubbing…" : "Idle")
                .font(.caption)
