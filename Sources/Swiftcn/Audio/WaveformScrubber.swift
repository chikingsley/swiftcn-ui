...
#Preview("Waveform") {
    SCPreview {
        SCWaveform(
            data: (0..<64).map { 0.15 + abs(sin(Double($0) * 0.35)) * 0.75 },
            height: 100
        )
        .padding()
    }
...
#Preview("Waveform · bar click") {
    @Previewable @State var tapped = "Tap a bar"
    SCPreview {
        VStack(spacing: 16) {
            SCWaveform(
                data: (0..<48).map { 0.2 + abs(sin(Double($0) * 0.5)) * 0.6 },
                height: 80,
                onBarClick: { index, value in
                    tapped = "Bar \(index): \(String(format: "%.2f", value))"
                }
            )
            Text(tapped)
                .font(.caption)
...
#Preview("StaticWaveform") {
    SCPreview {
        VStack(spacing: 20) {
            SCStaticWaveform(bars: 40, seed: 42, height: 80)
            SCStaticWaveform(bars: 60, seed: 7, height: 80)
        }
        .padding()
    }
}

#Preview("ScrollingWaveform") {
    SCPreview {
        VStack(spacing: 20) {
            SCScrollingWaveform(speed: 30, barWidth: 3, barGap: 2, height: 80)
            SCScrollingWaveform(
                speed: 60,
                data: (0..<24).map { 0.2 + abs(sin(Double($0) * 0.6)) * 0.7 },
                height: 80
            )
        }
        .padding()
...
#Preview("AudioScrubber") {
    @Previewable @State var currentTime: TimeInterval = 32
    let duration: TimeInterval = 100
    SCPreview {
        VStack(spacing: 12) {
            SCAudioScrubber(
                data: (0..<80).map { 0.2 + abs(sin(Double($0) * 0.3)) * 0.65 },
                currentTime: currentTime,
                duration: duration,
                onSeek: { currentTime = $0 },
                height: 100
            )
            Text("\(SCScrubBarTimeLabel.timestamp(currentTime)) / \(SCScrubBarTimeLabel.timestamp(duration))")
                .font(.caption)
                .monospacedDigit()
                .foregroundStyle(.secondary)
        }
