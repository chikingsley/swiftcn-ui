...
#Preview("Progress · values") {
    SCPreview {
        VStack(spacing: 16) {
            ForEach([0.0, 25, 50, 75, 100], id: \.self) { value in
                SCProgress(value: value)
            }
        }
        .frame(maxWidth: 320)
    }
...
#Preview("Progress · label and custom composition") {
    @Previewable @State var value = 56.0

    SCPreview {
        VStack(spacing: 20) {
            SCProgress(value: value, accessibilityLabel: "Upload progress") {
...
#Preview("Progress · indeterminate and native style") {
    SCPreview {
        VStack(spacing: 20) {
            SCProgress(value: nil, accessibilityLabel: "Loading")
