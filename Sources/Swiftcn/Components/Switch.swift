    ...
    SCPreview {
        VStack(spacing: 12) {
            ...
            Toggle("Wi-Fi", isOn: $wifi)
                .toggleStyle(.scSwitch(size: .small))
            HStack {
                Text("Share across devices")
                Spacer()
                SCSwitch("Share across devices", isOn: $wifi)
            }
        }
    }
}

#Preview("Switch · invalid and disabled") {
    SCPreview {
        VStack(spacing: 12) {
            SCSwitch("Invalid", isOn: .constant(false), isInvalid: true)
            SCSwitch("Disabled", isOn: .constant(true), isDisabled: true)
        }
    }
}
