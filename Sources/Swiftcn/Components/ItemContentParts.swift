...
#Preview("Item · parts") {
    SCPreview {
        SCItemGroup(spacing: 12) {
            SCItem(variant: .outline) {
                SCItemHeader {
                    Text("Design system").font(.caption.weight(.medium))
                    SCBadge("Active", variant: .secondary)
                }
                SCItemMedia(variant: .icon, isDecorative: true) {
                    Image(systemName: "shippingbox")
                }
                SCItemContent {
                    SCItemTitle("Component library")
                    SCItemDescription("Reusable native SwiftUI components for macOS and iPadOS.")
                }
                SCItemActions {
                    Button("Open") {}
                ...
                SCItemFooter {
                    Text("Updated 5 minutes ago").scMuted()
                }
            }
            SCItemSeparator()
            SCItem(variant: .muted, size: .sm) {
                SCItemContent {
                    SCItemTitle("Muted small item")
                    SCItemDescription("Every official part can be composed independently.")
                }
            }
        }
    }
...
#Preview("Item · native interactions") {
    @Previewable @State var action = "No action yet"

    SCPreview {
        VStack(spacing: 12) {
            SCItemButton(
                variant: .outline,
                action: { action = "Button activated" },
                content: {
                    SCItemMedia(variant: .icon, isDecorative: true) {
                        Image(systemName: "bell")
                    }
                    SCItemContent {
                        SCItemTitle("Notifications")
                        SCItemDescription("A real native button item.")
                    }
                }
            )
            SCItemLink(
                destination: URL(string: "https://ui.shadcn.com") ?? URL(fileURLWithPath: "/"),
                size: .sm
            ) {
                SCItemContent {
                    SCItemTitle("Open shadcn")
                }
                SCItemActions {
