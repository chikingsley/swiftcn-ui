...
#Preview("Drawer · composed bottom") {
    SCPreview {
        SCDrawer(showSwipeHandle: true) {
            SCDrawerTrigger("Open drawer")
...
#Preview("Drawer · snap points") {
    @Previewable @State var isPresented = false
    @Previewable @State var snapPoint: SCDrawerSnapPoint? = .fraction(0.3)

    SCPreview {
        Button("Open snap drawer") { isPresented = true }
            .buttonStyle(.sc(.outline))
            .frame(maxWidth: .infinity, maxHeight: .infinity)
    }
    .scDrawer(
        ...
        snapPoints: [.fraction(0.3), .fraction(0.6), .full],
        snapPoint: $snapPoint
    ) {
        SCDrawerContent {
            SCDrawerHeader {
                SCDrawerTitle("Snap points")
                SCDrawerDescription("Drag between three real heights.")
            }
...
#Preview("Drawer · side") {
    SCPreview {
        SCDrawer(
            ...
            SCDrawerTrigger("Open side drawer")
                .buttonStyle(.sc(.outline))
        } content: {
            SCDrawerContent {
                SCDrawerHeader {
                    SCDrawerTitle("Account")
                    SCDrawerDescription("A right-edge drawer.")
                }
                Spacer()
                SCDrawerFooter {
                    SCDrawerClose()
