...
#Preview("Button · variants") {
    @Previewable @State var lastAction = "Choose a variant."

    SCPreview {
        VStack(spacing: 12) {
            Button("Default") { lastAction = "Default" }.buttonStyle(.sc())
            Button("Destructive") { lastAction = "Destructive" }.buttonStyle(.sc(.destructive))
            Button("Outline") { lastAction = "Outline" }.buttonStyle(.sc(.outline))
            Button("Secondary") { lastAction = "Secondary" }.buttonStyle(.sc(.secondary))
            Button("Ghost") { lastAction = "Ghost" }.buttonStyle(.sc(.ghost))
            Button("Link") { lastAction = "Link" }.buttonStyle(.sc(.link))
...
#Preview("Button · sizes") {
    @Previewable @State var lastAction = "Choose a size."

    SCPreview {
        VStack(spacing: 12) {
            Button("Small") { lastAction = "Small" }.buttonStyle(.sc(.outline, size: .sm))
            Button("Default") { lastAction = "Default" }.buttonStyle(.sc(.outline))
            Button("Large") { lastAction = "Large" }.buttonStyle(.sc(.outline, size: .lg))
            Button {
                lastAction = "Icon"
            } label: {
...
#Preview("Button · states") {
    @Previewable @State var lastAction = "Choose the enabled action."
