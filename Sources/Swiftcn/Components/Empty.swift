...
#Preview("Empty · composed") {
    ...
    SCPreview {
        SCEmpty {
            SCEmptyHeader {
                SCEmptyMedia(variant: .icon) {
                    Image(systemName: "folder")
                }
                SCEmptyTitle("No projects yet")
                SCEmptyDescription {
                    Text("Create a project or import an existing workspace.")
                }
            }
            SCEmptyContent {
                HStack(spacing: 8) {
                    Button("Create project") { lastAction = "Create" }
                        .buttonStyle(.sc())
                    Button("Import project") { lastAction = "Import" }
        ...
        .background(Theme.default.muted.opacity(0.4))
        .clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
    }
}

#Preview("Empty · convenience") {
    @Previewable @State var cleared = false

    SCPreview {
        SCEmpty(
            cleared ? "Filters cleared" : "No results",
            systemImage: "magnifyingglass",
            description: "Try adjusting your search or removing filters."
        ) {
            Button("Clear filters") { cleared = true }
